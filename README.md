...
- 🎯 **Pick dot rings** — press & hold the picker dot: the **middle circle is the current color**
  (the enlarged pick indicator — translucent so the box shows through when semi-transparent),
  surrounded by two **touching, equal-width rings**. **Inner = saturation** — the C / white / black
  triangle perimeter wrapped into a full gradient ring (top = current color, left = white, right =
  black, bottom = gray); rotating around the dot walks that loop (hue preserved, marker dot shows
  the position). **Outer = alpha** — a checkerboard transparency ring whose arc shows the color at
  its alpha. Rotating ≥10° engages a ring, which then stays locked until release (drag through the
  center circle to switch rings — a chord across the sat ring passes through the alpha band, so
  locking keeps it from stealing the drag); release folds them back. Semi-transparent colors show
  an 8-digit `#RRGGBBAA` hex
...
| **Press & hold the pick dot** | middle circle = current color + two touching rings — inner = saturation (gradient: top color, left white, right black, bottom gray), outer = alpha (checkerboard); rotate ≥10° to engage a ring (locks until release; drag through the center to switch), release to fold |
